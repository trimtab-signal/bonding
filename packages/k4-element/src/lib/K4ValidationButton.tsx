...
export default function K4ValidationButton({ onValid, onInvalid, label = 'VALIDATE K₄', className = '' }: K4ValidationButtonProps) {
  ...
  const runValidation = useCallback(() => {
    ...
    const tick = () => {
      ...
      setCmdIndex(i);
      if (entry.label) {
        setEdges((prev) => prev.map((e) => (e.label === entry.label ? { ...e, checked: true, valid: entry.valid } : e)));
  ...
  return (
    <div className={`${styles.wrap} ${className}`}>
      <Kerberos4Element edges={edges} validationState={state} highlightEdges={state === 'valid' ? [[0, 1], [2, 3]] : undefined} />
      <div className={styles.terminal}>
        ...
        <div className={styles.history}>
          {history.map((line, i) => (
            <div key={i} className={`${styles.line} ${line.startsWith('→') ? (line.includes('DEGRADED') || line.includes('ERROR') ? styles.stderr : styles.stdout) : styles.user}`}>
              {line}
            </div>
          ))}
          {state === 'walking' && (
            <div className={styles.line + ' ' + styles.user}>
              $ {COMMANDS[cmdIndex]?.cmd ?? ''}<span className={styles.cursor} />
            </div>
          )}
        </div>
        <button className={`${styles.button} ${state === 'valid' ? styles.valid : state === 'invalid' ? styles.invalid : ''}`} onClick={runValidation} disabled={state === 'walking'}>
          {state === 'valid' ? '✓ K₄ VALID' : state === 'invalid' ? '✕ INVALID' : state === 'walking' ? 'VALIDATING...' : label}
        </button>
      </div>
    </div>
  );
