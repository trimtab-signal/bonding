...
import { createPing, respondToPing, recordCheckIn, getOrCreateBondId, getNearbyAtoms, isRateLimited, updateRateLimit } from '../services/game-loop.js';
import { boostCheckIn, penalizeRejection, adjustValence, distributeDividends, getValence } from '../services/valence.js';
  ...
  switch (msg.type) {
    ...
    case 'check_in': {
      ...
      if (Math.abs(Date.now() - timestamp) > 300000) {
        socket.emit('message', JSON.stringify({
          type: 'error', code: 'STALE_PROOF', message: 'Location proof expired',
        } satisfies ServerMessage));
        break;
      }
      ...
      if (isRateLimited(userId)) {
        socket.emit('message', JSON.stringify({
          type: 'error', code: 'RATE_LIMITED', message: 'Please wait 2 minutes between check-ins',
        } satisfies ServerMessage));
        break;
      }
      ...
      const proofValid = await verifySignature(user.rows[0].public_key_jwk, proofPayload, signature);
      if (!proofValid) {
        socket.emit('message', JSON.stringify({
          type: 'error', code: 'INVALID_SIGNATURE', message: 'Invalid location proof signature',
        } satisfies ServerMessage));
        break;
      }

      // Record check-in with PostGIS location
      await recordCheckIn(userId, msg.zoneId, geohashPrefix, lat, lng, msg.locationProof.witnessedBy, energyLevel);
      ...
      io.emit('message', JSON.stringify({
        type: 'zone_update', userId, zoneId: msg.zoneId,
      } satisfies ServerMessage));

      socket.emit('message', JSON.stringify({
        type: 'check_in_confirmed',
        checkIn: { userId, zoneId: msg.zoneId, locationProof: msg.locationProof, timestamp: Date.now(), energyLevel },
      } satisfies ServerMessage));
      break;
    }
    ...
    case 'provide_witness': {
      ...
      if (msg.claimerId === userId) {
        socket.emit('message', JSON.stringify({
          type: 'error', code: 'SELF_WITNESS', message: 'Cannot witness your own check-in',
        } satisfies ServerMessage));
        break;
      }
      ...
      if (msg.nonce && !isNonceValid(msg.nonce)) {
        socket.emit('message', JSON.stringify({
          type: 'error', code: 'REPLAY_REJECTED', message: 'Nonce already used or expired',
        } satisfies ServerMessage));
        break;
      }
      ...
      const witnessValid = await verifySignature(witness.rows[0].public_key_jwk, witnessPayload, msg.signature);
      if (!witnessValid) {
        socket.emit('message', JSON.stringify({
          type: 'error', code: 'INVALID_WITNESS', message: 'Witness signature invalid',
        } satisfies ServerMessage));
        break;
      }
