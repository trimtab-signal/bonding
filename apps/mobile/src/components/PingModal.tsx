  ...
  return (
    <Modal open={true} onClose={() => {}}>
      {pendingPings.map(ping => (
        <div key={ping.pingId} style={{ textAlign: 'center', padding: '0.5rem 0', display: 'flex', flexDirection: 'column', gap: '1rem', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', width: '100%' }}>
            <Button variant="ghost" fullWidth onClick={() => onReject(ping.pingId)}>Decline</Button>
            <Button variant="primary" fullWidth onClick={() => onAccept(ping.pingId)}>Accept</Button>
          </div>
        </div>
      ))}
    </Modal>
