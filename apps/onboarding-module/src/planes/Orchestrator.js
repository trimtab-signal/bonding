...
export default function OnboardingModule({ mode, resume, onComplete, onExit, onBack, adapters }) {
    ...
    useEffect(() => {
        if (phase === 'loading' || phase === 'complete' || phase === 'exited')
            return;
    ...
    const handleExit = useCallback((step) => {
        clearSession();
        onExit?.(step);
    }, [onExit]);
    const handleBack = useCallback((from) => {
        onBack?.(from);
        if (from === 'fleet-status')
            setPhase('trim-tabs');
        else if (from === 'trim-tabs')
            setPhase('magnum-walk');
        else
            handleExit('root');
    }, [onBack, handleExit]);
    const runAdapters = useCallback((plane) => {
        if (!adapters)
            return;
        const { k4, meatspace, phos } = adapters;
        if (!k4?.endpoint && !meatspace?.groundTruthPath && !phos?.endpoint)
            return;
        Promise.allSettled([
    ...
    const handlePlaneComplete = useCallback((data) => {
        persistPayload(data);
    ...
    const handleSkip = useCallback(() => {
        if (mode === 'full')
            ...
            handleComplete({ completed: true, plane: 'magnum-walk', timestamp: Date.now() });
    }, [mode, handleComplete]);
    const handleMagnumComplete = useCallback((_data) => {
        handlePlaneComplete({ completed: false, plane: 'magnum-walk', timestamp: Date.now() });
    }, [handlePlaneComplete]);
    const handleTrimTabsComplete = useCallback((_data) => {
    ...
    return (_jsx("div", { className: styles.moduleRoot, children: _jsxs(AnimatePresence, { mode: "wait", children: [(phase === 'magnum-walk' || phase === 'loading') && (_jsx(MagnumWalk, { onComplete: handleMagnumComplete, onSkip: handleSkip }, "magnum-walk")), phase === 'trim-tabs' && mode === 'full' && _jsx(TrimTabs, { onComplete: handleTrimTabsComplete, onBack: handleTrimBack }, "trim-tabs"), phase === 'fleet-status' && mode === 'full' && _jsx(FleetStatus, { onBack: handleFleetBack }, "fleet-status")] }) }));
}
