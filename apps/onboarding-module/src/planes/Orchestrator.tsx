...
export default function OnboardingModule({
  ...
  const runAdapters = useCallback(
    (plane: 'magnum-walk' | 'trim-tabs' | 'fleet-status') => {
      if (!adapters) return;
  ...
  return (
    <div className={styles.moduleRoot}>
      <AnimatePresence mode="wait">
        ...
        {phase === 'trim-tabs' && mode === 'full' && (
          <TrimTabs key="trim-tabs" onComplete={handleTrimTabsComplete} onBack={handleTrimBack} />
        )}
        {phase === 'fleet-status' && mode === 'full' && (
          <FleetStatus key="fleet-status" onBack={handleFleetBack} />
        )}
      </AnimatePresence>
    </div>
  );
