...
export function TrimTabs() {
  ...
  useEffect(() => {
    ...
    if (idx === 0) return; // first render guard

    const delay = 600 + Math.random() * 900;
    const t = setTimeout(() => {
      setSteps((prev) => prev.map((s, i) => (i === idx - 1 ? { ...s, status: 'done' as Status } : s)));
    }, delay);
            ...
            return (
              ...
              >
                <div className={styles.stepTop}>
                  ...
                  <span className={styles.stepId}>#{i + 1}</span>
                </div>
