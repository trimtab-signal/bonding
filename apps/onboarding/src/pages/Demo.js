...
export const Demo = () => {
        ...
        const physics = () => {
            ...
            for (let i = 0; i < newAtoms.length; i++) {
                for (let j = i + 1; j < newAtoms.length; j++) {
                    ...
                    const existing = existingBonds.find(bd => (bd.a === i && bd.b === j) || (bd.a === j && bd.b === i));
                    if (d < bondThreshold) {
                        if (existing) {
                        ...
                        if (faded > 0.01)
                            newBonds.push({ ...existing, strength: faded });
                    }
                }
            }
            // Bond attraction
    ...
    return (_jsx(Layout, { showFooter: false, children: _jsx("section", { className: styles.section, children: _jsxs("div", { className: styles.inner, children: [_jsxs("div", { className: styles.header, children: [_jsx("h2", { className: styles.title, children: "\uD83E\uDDEA The Living Molecule" }), _jsxs("p", { className: styles.desc, children: ["Atoms drift, bond, and react. ", _jsx("strong", { children: "Click" }), " to add, ", _jsx("strong", { children: "drag" }), " to bond. Switch zones to change the vibe."] }), tutorialStep === -1 && (_jsx("button", { className: styles.replayBtn, onClick: () => { try {
                                    localStorage.setItem('bonding_demo_tutorial_done', 'false');
                                }
                                catch { } setTutorialStep(0); }, children: "\uD83D\uDD04 Show Tutorial Again" }))] }), _jsxs("div", { className: styles.canvasWrap, ref: containerRef, children: [_jsx("canvas", { ref: canvasRef, className: styles.canvas, onClick: handleClick, onMouseDown: handleMouseDown, onMouseMove: handleMouseMove, onMouseUp: handleMouseUp, onMouseLeave: handleMouseUp }), step && (_jsx("div", { className: styles.tutorialOverlay, children: _jsxs("div", { className: styles.tooltip, children: [_jsxs("div", { className: styles.tooltipHeader, children: [_jsxs("span", { className: styles.tooltipStep, children: ["Step ", tutorialStep + 1, " of ", totalSteps] }), _jsx("button", { className: styles.tooltipSkip, onClick: persistDone, children: "Skip" })] }), _jsx("h3", { className: styles.tooltipTitle, children: step.title }), _jsx("p", { className: styles.tooltipText, children: step.text }), step.action && _jsx("div", { className: styles.tooltipAction, children: step.action }), tutorialStep === totalSteps - 1 && (_jsx("button", { className: styles.tooltipNext, onClick: persistDone, children: "\uD83C\uDF89 Start Playing" }))] }) })), _jsxs("div", { className: styles.controls, children: [ZONES.map((z, i) => (_jsx("button", { className: `${styles.zoneBtn} ${i === zoneIndex ? styles.active : ''}`, onClick: () => { setZoneIndex(i); if (tutorialStep === 3)
                                            goToStep(4); }, style: { '--zone-color': z.color }, children: z.label }, z.id))), _jsx("button", { className: styles.resetBtn, onClick: () => { initAtoms(); if (tutorialStep === 4)
                                            goToStep(5); }, children: "\u27F3 Reset" })] })] })] }) }) }));
};
