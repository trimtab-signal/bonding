  ...
  return (
    <section id="mechanics">
      <div>
        ...
        <p className="lead">
          Bonds track real co-presence — not likes, not messages. Every check-in with a bonded atom
        ...
        <div className="bond-types">
          {BOND_TYPES.map((bt) => (
            <div className="bond-type" key={bt.title}>
              <h3>{bt.title}</h3>
              <p>{bt.desc}</p>
              <span className="tag" style={{ background: bt.tagColor + '20', color: bt.tagColor }}>
                {bt.tag}
              </span>
            </div>
          ))}
        ...
        <div style={{ marginTop: 60 }}>
          ...
          <p className="lead" style={{ marginBottom: 32 }}>
            BONDING reads PHOS health data (on-device only) to suggest the best zone for your
            current capacity. No data leaves your device unless you opt in.
          </p>
          <div
            style={{
              display: 'grid',
