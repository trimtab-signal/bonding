      ...
      <div className={styles.inner}>
        ...
        <div className={styles.kpiGrid}>
          ...
          <div className={styles.kpiCard}>
            ...
            <div className={styles.kpiLabel}>TOTAL BONDS</div>
          </div>
          <div className={styles.kpiCard}>
            <div className={styles.kpiValue}>312</div>
            <div className={styles.kpiLabel}>PSI MSG/MIN</div>
          </div>
          <div className={styles.kpiCard}>
            <div className={styles.kpiValue}>1.2s</div>
            <div className={styles.kpiLabel}>RESPONSE LAT</div>
          </div>
        </div>
        <div className={styles.terminal}>
          <div className={styles.terminalHeader}>
            ...
            <span className={styles.terminalTitle}>meatspace@k4-fleet:~$</span>
          </div>
          <div className={styles.terminalBody}>
            {history.map((cmd, i) => (
              <div key={i}>
                <div className={styles.userInput}>$ {cmd.input}</div>
                <div className={cmd.type === 'error' ? styles.stderr : styles.stdout}>
                  {cmd.output.split('\n').map((line, j) => (
                    <div key={j}>{line || '\u00A0'}</div>
                  ))}
                </div>
        ...
        <a href="https://bonding.p31ca.org/#/" className={styles.backLink}>
          ← Back to Bonding
        </a>
      </div>
    </div>
